...
9. **Campos editoriais sobrevivem ao upsert?** **CONFIRMADO — sim.** O código
   implantado da edge fn `campaigns` (v13, recuperado do Supabase — ver
   `docs/auditoria/edge-function-campaigns.md`) faz `upsert(rows, {onConflict:"id"})`
   com um payload que **não inclui** `verdict`, `tl_score` nem `used_in` → essas
   colunas **não** entram no `SET` do conflito e são **preservadas**.
10. **O upsert pode apagar/substituir datas válidas?** **CONFIRMADO.** Para o **mesmo
    `id`**, o payload sobrescreve `vigencia_inicio/fim`, `status`, `first_seen`,
    `observed_at`, etc. Para `id` **diferente** (o que ocorre quando a nova extração
    produz outra `vigencia_fim`, inclusive por erro de ano), **cria linha nova** — é
    a raiz da duplicidade. (`makeId` = `origem-destino-tipo-vigencia_fim`; `first_seen
    = news_raw.published_at`; `origin` sempre `"auto"`.)

> **Atualização (evidência da edge function).** Os itens 9–10 acima foram confirmados
> pela recuperação do código implantado da edge fn `campaigns` — antes marcados como
> "não foi possível confirmar". Detalhe completo (prompt, schema, `makeId`, `on
> conflict`, `first_seen`) em `docs/auditoria/edge-function-campaigns.md`.
...
8. **Decisões de negócio pendentes** (do mapa funcional): motor canônico (Forecast ×
   Predict); a data canônica do evento (ver correção abaixo); e a política de
   plausibilidade/deduplicação.

**A próxima fase deve começar por dados, não por modelo:** (a) resolver uma **data
canônica do evento** por prioridade entre **datas de evento** (`vigencia_inicio` >
`data_anuncio` > `vigencia_fim`), com **validação de plausibilidade** contra as **datas
de proveniência**; (b) deduplicar por **identidade estável** (não `id`-com-`vigencia_fim`);
(c) completar a normalização e remover séries-lixo; (d) só então recalibrar Forecast/
Predict e reconciliá-los. Sem (a)–(c), qualquer modelo continuará prevendo sobre uma
cronologia falsa.

> ## ⚠ Correção canônica desta conclusão (revisão pós-arquitetura)
>
> Uma versão anterior desta auditoria recomendava **"datar a campanha por
> `first_seen`/data de publicação da notícia"**. **Essa recomendação NÃO deve ser
> adotada automaticamente** e fica **corrigida** aqui:
>
> - **Datas de evento** (comercial): `vigencia_inicio`, `vigencia_fim`, `data_anuncio`
>   — só estas podem ancorar a série de produção.
> - **Datas de proveniência** (ingestão/processamento): `data_publicacao`
>   (=`first_seen`, que a edge fn preenche de `news_raw.published_at`), `observed_at`,
>   `created_at` — **nunca** ancoram a série; servem para **validar plausibilidade** e
>   acionar **bloqueio / reprocessamento / revisão humana**.
> - Proveniência **não substitui** automaticamente a data do evento, **não entra**
>   direto na série, e **não** transforma campanha antiga em recente sem evidência
>   textual.
>
> **Exemplo obrigatório** (o caso `livelo→connectmiles`):
> ```
> vigencia_fim    = 2023-12-12   (data de evento — provavelmente fabricada)
> data_publicacao = 2026-07-12   (proveniência = first_seen)
> first_seen      = 2026-07-12
> observed_at     = 2026-07-13
> → temporal_status        = suspect_year
>   include_in_prediction  = false
>   requires_reprocessing   = true
>   requires_human_review   = true
> ```
> **Não** corrigir automaticamente para `2026-07-12`. A política formal está em
> `ADR-RADAR-010-validacao-temporal.md`, na arquitetura §6/§27f e na
> `docs/RECONCILIACAO-AUDITORIAS-RADAR.md`.

---

*Fim da auditoria. Etapa de leitura — nenhum código, banco, campanha, backfill,
snapshot, parâmetro ou publicação foi alterado. Ver a correção canônica acima e a
`docs/RECONCILIACAO-AUDITORIAS-RADAR.md`.*
