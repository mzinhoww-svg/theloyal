...
export async function collectShoppingAction(_prev: ActionState, fd: FormData): Promise<ActionState> {
  ...
  const diagnose = String(fd.get("diagnose") || "") === "1";
  ...
  try {
    const res = await fetch(`https://api.github.com/repos/${repo}/actions/workflows/shopping-collect.yml/dispatches`, {
      ...
      body: JSON.stringify({ ref, inputs: { mock: mock ? "true" : "false", diagnose: diagnose ? "true" : "false" } }),
    });
    if (res.status !== 204) {
      const detail = await res.text().catch(() => "");
      return { ok: false, message: `dispatch falhou (${res.status}) ${detail.slice(0, 160)}` };
    }
    const label = diagnose ? "diagnóstico" : mock ? "mock" : "live";
    return { ok: true, message: `coleta disparada no Actions (${label}) — ${diagnose ? "artefato em Actions ao terminar" : "acompanhe em Execuções"}` };
  } catch (e) {
