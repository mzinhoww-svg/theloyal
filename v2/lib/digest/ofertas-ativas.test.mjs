...
import { selecionarOfertasAtivas, selecionarCartoesBancos, BANCOS_ORIGEM } from './ofertas-ativas.mjs';
...
test('vários itens cruzando bandas de veredito diferentes → todos entram, sem corte de veredito', () => {
  const campanhas = [
    { id: 'a', tipo: 'transferencia', origem_code: 'livelo', destino_code: 'smiles', estado: 'ativa', tier: 1, tl_score_bruto: 90, veredito_bruto: 'Vale agir', percentual: 100, vigencia_fim: '2026-08-01T00:00:00-03:00' },
    { id: 'b', tipo: 'transferencia', origem_code: 'esfera', destino_code: 'azul', estado: 'ativa', tier: 1, tl_score_bruto: 75, veredito_bruto: 'Vale olhar', percentual: 90, vigencia_fim: '2026-08-05T00:00:00-03:00' },
    { id: 'c', tipo: 'compra', origem_code: 'brl', destino_code: 'smiles', estado: 'ultimos_dias', tier: 1, tl_score_bruto: 55, veredito_bruto: 'Só para casos específicos', percentual: 40, vigencia_fim: '2026-07-17T23:59:00-03:00' },
    { id: 'd', tipo: 'clube', origem_code: 'azul', destino_code: 'azul', estado: 'ativa', tier: 1, tl_score_bruto: 48, veredito_bruto: 'Esperaria', percentual: 20, vigencia_fim: null },
    { id: 'e', tipo: 'transferencia', origem_code: 'livelo', destino_code: 'hilton', estado: 'ativa', tier: 1, tl_score_bruto: 30, veredito_bruto: 'Evitaria', percentual: 50, vigencia_fim: '2026-08-01T00:00:00-03:00' },
  ];
...
test('destino null (lado único, ex.: compra de pontos) preservado como null', () => {
  const campanhas = [{ id: 'x', tipo: 'compra', origem_code: 'smiles', destino_code: null, estado: 'ativa', tier: 1, tl_score_bruto: 60, veredito_bruto: 'Só para casos específicos', percentual: null, vigencia_fim: null }];
  const r = selecionarOfertasAtivas(campanhas);
...
test('exclui item que não passa os 3 portões (morto/tier2/sem conta) — mesma disciplina de passaTresPortoes', () => {
  const campanhas = [
    { id: 'morto', tipo: 'compra', estado: 'encerrada', tier: 1, tl_score_bruto: 90, veredito_bruto: 'Vale agir' },
    { id: 'tier2', tipo: 'compra', estado: 'ativa', tier: 2, tl_score_bruto: 90, veredito_bruto: 'Vale agir' },
    { id: 'sem-conta', tipo: 'compra', estado: 'ativa', tier: 1, tl_score_bruto: null, veredito_bruto: 'Não confirmado' },
  ];
  const r = selecionarOfertasAtivas(campanhas);
...
test('veredito_bruto desconhecido lança (sem fallback silencioso, mesma disciplina de mapVeredito)', () => {
  const campanhas = [{ id: 'x', tipo: 'compra', estado: 'ativa', tier: 1, tl_score_bruto: 60, veredito_bruto: 'Rótulo inventado' }];
...
test('Cartões & bancos: transferencia com origem_code fora da lista curada é excluída', () => {
  const campanhas = [{ id: 'x', tipo: 'transferencia', origem_code: 'programa_desconhecido', estado: 'ativa', tier: 1, tl_score_bruto: 80 }];
  const r = selecionarCartoesBancos(campanhas);
  assert.equal(r.omitido, true);
});
