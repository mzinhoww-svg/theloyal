// node --test — CPM a partir do custo-base (M2, D-032).
// Determinismo, formula CPM = custo / ((1 + B/100) * ratio), fronteira null.
// Numeros ancorados nas campanhas reais lidas do banco (read-only, jul/2026);
// ver v2/M2/PROPOSTA-CUSTO-BASE.md.

import { test } from 'node:test';
import assert from 'node:assert/strict';
import { cpmDeCustoBase } from './custo-base.mjs';

test('livelo->azul 115% com custo-base Livelo=30 -> CPM 13,95', () => {
  // 30 / (1 + 1,15) = 30 / 2,15 = 13,953...
  assert.equal(cpmDeCustoBase(30, 115), 13.95);
});

test('esfera->latampass 120% com custo-base Esfera=35 -> CPM 15,91', () => {
  // 35 / 2,20 = 15,909...
  assert.equal(cpmDeCustoBase(35, 120), 15.91);
});

test('smiles piso 21, bonus 0 -> CPM 21,00 (base pura)', () => {
  assert.equal(cpmDeCustoBase(21, 0), 21);
});

test('ratio nao-1:1 eleva o CPM (caveat ConnectMiles): ratio 0,5 dobra', () => {
  const r1 = cpmDeCustoBase(30, 40, 1);      // 30 / 1,4 = 21,43
  const rMeio = cpmDeCustoBase(30, 40, 0.5); // 30 / 0,7 = 42,86
  assert.equal(r1, 21.43);
  assert.equal(rMeio, 42.86);
  assert.ok(rMeio > r1);
});

test('determinismo: mesma entrada -> mesma saida', () => {
  assert.equal(cpmDeCustoBase(35, 130), cpmDeCustoBase(35, 130));
});

test('fronteira null (nao chuta, INV-03): custo/bonus/ratio invalidos', () => {
  assert.equal(cpmDeCustoBase(null, 100), null);
  assert.equal(cpmDeCustoBase(0, 100), null);
  assert.equal(cpmDeCustoBase(-5, 100), null);
  assert.equal(cpmDeCustoBase(30, -10), null);
  assert.equal(cpmDeCustoBase(30, NaN), null);
  assert.equal(cpmDeCustoBase(30, 100, 0), null);
  assert.equal(cpmDeCustoBase(30, 100, null), null);
});

test('bonus 0 devolve o proprio custo-base', () => {
  assert.equal(cpmDeCustoBase(28, 0), 28);
});
